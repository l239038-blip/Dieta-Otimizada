...
function main()
    ...
    for alimento in alimentos
        println(
            ...
            "Gorduras: $(alimento.gordura) g " *
            "Carboidratos: $(alimento.carboidrato) g " *
            "Preço: R\$ $(round(alimento.preco, digits=2)) " *
            "Refeição: $(alimento.refeicao) "
        )
        println()
    end
    ...
    for refeicao in REFEICOES
        ...
        @constraint(model,
            sum(y[i] for i in idx) >= MIN_ALIMENTOS
        )
